...
#print header['TSTART']
	...
	plt.scatter(SRA,SDEC,marker='.', edgecolors='none', s=8,label="sun")
	plt.xlabel('Right Ascension')
	plt.ylabel('Decliniation')
	#params = {'legend.fontsize': 12,'legend.linewidth': 2, 'legend.numpoints':1}
	#plt.rcParams.update(params)
	#plt.legend(bbox_to_anchor=(1,1), loc=1, borderaxespad=0.)
	#plt.grid(True)
	plt.savefig("graph_RA_DEC_%s.png" %(filename))
	plt.close()
	...
	plt.rc('text', usetex=True)
	plt.rc('font', family='serif')
	plt.scatter(log10time,a,marker='.', edgecolors='none', s=8,label="sun")
	plt.xlabel('log10timephoton')
	plt.ylabel('a:angle between photon and sun')
	#params = {'legend.fontsize': 12,'legend.linewidth': 2, 'legend.numpoints':1}
	#plt.rcParams.update(params)
	#plt.legend(bbox_to_anchor=(1,1), loc=1, borderaxespad=0.)
	#plt.grid(True)
	plt.savefig("graph_a_log10time_%s.png" %(filename))
	plt.close()
	...
	for i in range(len(a)):
			if a[i] > angle:
				aa.append(a[i])
				l.append(log10time[i])


	histogram = plt.hist(l,80, log=True)
	plt.title('Histogram log10time, a more than 15')
	plt.xlabel('log10time')
	plt.ylabel('counts')
	plt.savefig("histo_morethan15_%s.png" %(filename))
	plt.close()
	...
	histogram = plt.hist(l,80, log=True)
	plt.title('Histogram log10time, a less than 15')
	plt.xlabel('log10time')
	plt.ylabel('counts')
	plt.savefig("histo_lessthan15_%s.png" %(filename))
	plt.close()
